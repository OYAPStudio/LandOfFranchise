  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 py-2">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-5">
            ...
            <div className="relative brands-dropdown-container">
              ...
              >
                {t.brands}
                <ChevronDown size={16} className={`ml-1 transition-transform duration-300 ${brandsDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
              
              {/* Dropdown Menu */}
              <div 
                id="brands-dropdown"
                className={`absolute left-0 mt-2 w-64 rounded-lg shadow-xl bg-white dark:bg-gray-800 ring-1 ring-black/5 overflow-hidden
                  transition-all duration-200 ease-in-out transform origin-top-left z-50
              ...
              >
                <div className="py-2">
                  {restaurants.map((restaurant, index) => (
                    ...
                    >
                      <div className="w-10 h-10 rounded-full overflow-hidden mr-3 flex-shrink-0 border-2 border-white dark:border-gray-700 shadow-sm transition-transform duration-200 group-hover:scale-110">
                        <Image 
                          src={restaurant.logo} 
                          alt={restaurant.name}
                          width={40}
                      ...
                      <div className="flex-1">
                        <p className="font-medium group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors duration-200">{restaurant.name}</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">{restaurant.cuisineType}</p>
                      </div>
                      <span className="text-amber-500 opacity-0 group-hover:opacity-100 transition-opacity duration-200">Visit &rarr;</span>
                    </Link>
                  ))}
                </div>
              </div>
        ...
        {/* Mobile Navigation */}
        {mobileMenuOpen && (
          <nav className="md:hidden pt-3 pb-2 space-y-1.5">
            <Link
              href={getLink('#home')}
            ...
            <div className="py-1.5">
              ...
              >
                {t.brands}
                <ChevronDown size={16} className={`ml-1 transform transition-transform duration-300 ${brandsDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
              ...
              <div className={`overflow-hidden transition-all duration-300 ease-in-out ${brandsDropdownOpen ? 'max-h-56 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                <div className={`pl-3 space-y-1 ml-2 ${
                  scrolled 
                  ...
                  {restaurants.map((restaurant) => (
                    <Link
                      ...
                      rel="noopener noreferrer"
                    >
                      <div className="w-8 h-8 rounded-full overflow-hidden mr-3 flex-shrink-0 border-2 border-white dark:border-gray-700 shadow-sm">
                        <Image 
                          src={restaurant.logo} 
                          alt={restaurant.name}
                          width={32}
